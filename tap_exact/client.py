...
class ExactPaginator(BaseHATEOASPaginator):
    ...
    def get_next_url(self, response):
        ...
        if "link" in data.get("feed", {}).keys():
            link_dict = {}
            links = data["feed"]["link"]
            if type(links) == list:
                for record in data["feed"]["link"]:
...
class ExactStream(RESTStream):
    ...
    def url_base(self) -> str:
        ...
        return f"https://start.exactonline.nl/api/v1/{self.config['division']}"
    ...
    def get_url_params(self, context: Optional[dict], next_page_token) -> Dict[str, Any]:
        ...
        if self.replication_key:
            date_filter = f"Modified ge datetime'{start_date}'"
    ...
    def parse_response(self, response: requests.Response) -> Iterable[dict]:
        ...
        """
        data = self.xml_to_dict(response)["feed"]["entry"]
